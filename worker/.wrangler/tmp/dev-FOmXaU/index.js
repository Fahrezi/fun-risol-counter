...
var SESSION_COOKIE = "mmdg_session";
var SESSION_TTL_SECONDS = 7 * 24 * 3600;
...
__name(todayJakarta, "todayJakarta");
function dateJakarta(iso) {
  return new Date(new Date(iso).getTime() + 7 * 3600 * 1e3).toISOString().slice(0, 10);
}
__name(dateJakarta, "dateJakarta");
async function rolloverStock(db) {
  ...
  await db.batch([
    ...
    ).bind((/* @__PURE__ */ new Date()).toISOString(), today),
    db.prepare("UPDATE food_stock SET qty = 0, stock_date = ? WHERE stock_date <> ?").bind(today, today)
  ]);
  return today;
}
__name(rolloverStock, "rolloverStock");
app.post("/auth/verify", async (c) => {
  ...
  await c.env.DB.prepare("INSERT INTO sessions (token, created_at, expires_at) VALUES (?, ?, ?)").bind(token, now.toISOString(), expiresAt.toISOString()).run();
...
app.post("/customers", async (c) => {
  ...
  const customer = { id: crypto.randomUUID(), name: trimmed };
  await c.env.DB.prepare("INSERT INTO customers (id, name) VALUES (?, ?)").bind(customer.id, customer.name).run();
...
app.post("/foods", async (c) => {
  ...
  await c.env.DB.prepare("INSERT INTO foods (id, name, price, icon) VALUES (?, ?, ?, ?)").bind(food.id, food.name, food.price, food.icon).run();
  return c.json(food);
});
app.patch("/foods/:id", async (c) => {
  const id = c.req.param("id");
  const body = await c.req.json();
  const existing = await c.env.DB.prepare("SELECT id, name, price, icon FROM foods WHERE id = ?").bind(id).first();
  ...
  await c.env.DB.prepare("UPDATE foods SET name = ?, price = ?, icon = ? WHERE id = ?").bind(updated.name, updated.price, updated.icon, updated.id).run();
...
app.put("/stock/:foodId", async (c) => {
  ...
  ).bind(foodId, body.qty, today).run();
...
app.get("/orders", async (c) => {
  const { results } = await c.env.DB.prepare(`${ORDERS_SELECT} ORDER BY o.rowid DESC`).all();
...
app.post("/orders", async (c) => {
  ...
  const qtyByFoodId = /* @__PURE__ */ new Map();
  ...
  for (const [foodId, qty] of qtyByFoodId) {
    const food = await c.env.DB.prepare("SELECT id, name, price, icon FROM foods WHERE id = ?").bind(foodId).first();
    if (!food) return c.json({ message: `Unknown food ${foodId}` }, 400);
    const stock = await c.env.DB.prepare(
      "SELECT qty FROM food_stock WHERE food_id = ? AND stock_date = ?"
    ).bind(foodId, today).first();
  ...
  const statements = [
    ...
    ).bind(order2.id, order2.listName, order2.total, order2.createdAt),
    ...items.map(
      (i) => c.env.DB.prepare(
        "INSERT INTO order_items (order_id, food_id, name, price, qty) VALUES (?, ?, ?, ?, ?)"
      ).bind(order2.id, i.foodId, i.name, i.price, i.qty)
    ),
    ...items.map(
      (i) => c.env.DB.prepare(
...
app.delete("/orders/:id", async (c) => {
  const id = c.req.param("id");
  const today = await rolloverStock(c.env.DB);
  const order2 = await c.env.DB.prepare("SELECT id, created_at FROM orders WHERE id = ?").bind(id).first();
  const statements = [
    c.env.DB.prepare("DELETE FROM order_items WHERE order_id = ?").bind(id),
    c.env.DB.prepare("DELETE FROM orders WHERE id = ?").bind(id)
  ];
  if (order2 && dateJakarta(order2.created_at) === today) {
    const { results } = await c.env.DB.prepare(
...
// .wrangler/tmp/bundle-lAJOf7/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-lAJOf7/middleware-loader.entry.ts
